...
##Domain:## Domain: O -> P(S) is an Operator(Function) that defines for each Operator op in O, which states we can apply op on. Domain(op) = {s in S | op(s) != empty set }
##Succ:## Succ: S -> P(S) is an Operator(Function) that defines for each state s in S, which states we can get to from s by applying an Operator from O on s. (Successors set of s)

### What are we searching for ?
We search for a goal state Sg in G, that has a path from an Initial State, but wait a second, is that it ? NO! We can have variations of search problems, e.g. not all paths have the same "price", which leads to the following ##definition:## * A Cost function is defined as follows: Cost: { <s1, s2> | s1 in S & s2 in Succ(s1)} -> R
...
##Space Complexity:## O(b*D) wher b is the branching factor and D is the depth of the solution
...
##Space Complexity:## O(b*l) wher b is the branching factor and l is the depth we supplied
